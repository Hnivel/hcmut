...
void Heap<T>::reheapUp(int position)
{
    while (position > 0)
    {
        ...
        if (this->aLTb(this->elements[position], this->elements[parent])) // swap this->elements[position] and this->elements[parent]
        {
...
void Heap<T>::reheapDown(int position)
{
    while (position < count)
    {
        ...
        if (left < this->count && this->aLTb(elements[left], elements[largest]))
        {
            largest = left;
        }
        if (right < this->count && this->aLTb(elements[right], elements[largest]))
        {
            largest = right;
        }
        if (largest != position)
...
void Heap<T>::copyFrom(const Heap<T> &heap)
{
    ...
    this->deleteUserData = nullptr; // heap.deleteUserData -> nullptr

    // Copy items from heap:
    for (int idx = 0; idx < this->count; idx++) // heap.size() -> this->count
    {
