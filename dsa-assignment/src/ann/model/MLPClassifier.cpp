...
MLPClassifier::MLPClassifier(string cfg_filename, string sModelName, ILayer **seq, int size) : IModel(cfg_filename, sModelName)
{
    // layer to m_layers:
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double> *pLoader)
{
    ...
    ClassMetrics metrics(this->get_num_classes());
    ...
    //
    double_tensor results = metrics.get_metrics();
    this->set_working_mode(old_mode);
    return results;
}
...
{
    double_tensor gradient = this->m_pLossLayer->backward();
