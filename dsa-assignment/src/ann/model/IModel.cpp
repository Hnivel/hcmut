...
void IModel::fit(DataLoader<double, double> *pTrainLoader, DataLoader<double, double> *pValidLoader, unsigned int nepoch, unsigned int verbose)
{
    ...
    for (int epoch = 1; epoch <= this->m_nepoches; epoch++)
    {
        on_begin_epoch();
...
void IModel::on_begin_training(DataLoader<double, double> *pTrainLoader, DataLoader<double, double> *pValidLoader, unsigned int nepoch, int verbose)
{
    ...
    cout << "Start the training ..." << endl; // 1: delete later
}
