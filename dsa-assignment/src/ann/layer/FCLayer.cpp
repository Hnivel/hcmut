...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X)
{
    ...
    {
        Y = xt::linalg::tensordot(X, xt::transpose(this->m_aWeights), 1);
    }
    if (this->m_bUse_Bias == true)
    {
        Y += this->m_aBias;
    }
    return Y;
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    if (DY.dimension() == 1)
    {
        ...
        return xt::linalg::tensordot(xt::transpose(this->m_aWeights), DY, 1);
    }
    int nsamples = DY.shape()[0];
    this->m_unSample_Counter += nsamples;
    this->m_aGrad_W = xt::zeros<double>({this->m_aWeights.shape()[0], this->m_aWeights.shape()[1]});
    for (int i = 0; i < nsamples; i++)
    {
        auto DY_i = xt::view(DY, i);
        auto X_i_transpose = xt::transpose(xt::view(this->m_aCached_X, i));
        this->m_aGrad_W += xt::linalg::outer(DY_i, X_i_transpose);
    }
    if (this->m_bUse_Bias == true)
    {
        this->m_aGrad_b = xt::sum(DY, {0});
    }
    xt::xarray<double> DX = xt::linalg::tensordot(DY, this->m_aWeights, 1);
    return DX;
}
...
{
    string desc = fmt::format("{:<10s}, {:<15s}: {:<4d}, {:<4d}, {:<4d}", "FC", this->getname(), this->m_nNin, this->m_nNout, this->m_bUse_Bias);
    return desc;
}
